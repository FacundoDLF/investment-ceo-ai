...
### [ISSUE] Error 400 (output_parse_failed) en Groq por Chain of Thought en Tool Calls
* **Contexto:** En el sub-agente Scrappy (Ejecutor HFT) que usa modelos rápidos (como `openai/gpt-oss-20b` vía Groq), el LLM intentaba "razonar" su jugada ("We need to output a function call...") antes de llamar a la herramienta `scalp_action`.
* **Problema:** La API de Groq lanza un HTTP 400 `output_parse_failed` cuando se usa `tool_choice: 'auto'` y el modelo escupe texto (Chain of Thought) en lugar de invocar estrictamente el JSON de la herramienta. Esto causaba un fallback constante.
...
### [ISSUE] Descorrelación entre Budget y Target de Scrappy (Presupuesto $10000 con Target $20)
* **Contexto:** Al llamar a la tool `command_scrappy`, el CEO puede proporcionar un `budget` pero omitir el `target`, o los valores por defecto pueden estar desfasados.
* **Problema:** Se asignaba un presupuesto gigante (ej. $10000 USD) manteniendo la meta por defecto en $20, perdiendo la correlación esperada del 10% de ganancia por sprint de Scrappy. 
* **Solución:** En `state.service.ts` se ajustaron los valores por defecto a $200 de budget y $20 de target. Además, se añadió una lógica de fallback: si se actualiza el `budget` pero NO el `target`, el sistema auto-calcula el `target` dinámicamente como el 10% del nuevo presupuesto (`budget * 0.1`).
* **REGLA:** Al asignar fondos a sub-agentes de alto riesgo, los targets y loss limits deben ser escalables en porcentaje. Nunca dejes un número absoluto (fijo) si el input del capital es variable.
