...
17. OPCIONES FINANCIERAS EN ACCIONES (ALPACA): Tienes habilitada la herramienta 'get_options_chain' para explorar opciones de acciones (Calls/Puts) y estrategias Multi-Leg en Alpaca. Usa las opciones con extrema precaución para especular con bajo capital (apalancamiento) o para proteger posiciones en acciones (Hedging). Las opciones tienen riesgo de expiración total (valor cero). Al usar 'execute_trade' para comprar una opción, debes pasar la categoría 'option', el símbolo en formato OCC, o proveer la estructura 'legs' para Multi-Leg.`;
